...
public class DAOLembreteAcademico extends DAOLembreteSocial{
	
	// CRUD CREATE
	@Override
	public void inserirLembrete(Lembrete lembrete) {
		String create = "insert into Lembrete_Academico(codEvento,usuario,nota) values (?,?,?)";
		try {
	...
	public ArrayList<Lembrete> listarLembretes(){
		ArrayList<Lembrete> lembretesAcademicos = new ArrayList<>();
		String read = "select * from Lembrete_Academico";
		try {
			Connection con = conectar();
			...
			while(rs.next()) {
				...
				lembretesAcademicos.add(new Lembrete(codLembrete, EAcademico, user, nota));
			}
			con.close();
			return lembretesAcademicos;
		} catch (Exception e) {
			System.out.println(e);
			return null;
		}
	...
	public void selecionarLembrete(Lembrete lembrete) {
		String read2 = "select * from Lembrete_Academico where codLembrete = ?";
		try {
			Connection con = conectar();
	...
	public Lembrete verificarLembrete(Lembrete lembrete) {
		String read3 = "select * from Lembrete_Academico where codEvento = ? and usuario = ?";
	...
	public void alterarLembrete(Lembrete lembrete) {
		String update = "update Lembrete_Academico set nota=? where codLembrete=?";
		try {
			Connection con = conectar();
	...
	public void deletarLembrete(Lembrete lembrete) {
		String delete = "delete from Lembrete_Academico where codLembrete = ?";
		try {
			Connection con = conectar();
